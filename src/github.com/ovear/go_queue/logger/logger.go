//代码部分来自 github.com/donnie4w/go-logger/logger
//对 golang 中 log 的封装
package logger
...
func init() {
	fmt.Println("start init log system...")
	//打印到控制台中
	SetConsole(true)
...
func _debug(v ...interface{}) {
	...
	if logLevel <= DEBUG {
		if logObj != nil {
			logObj.lg.Output(3, fmt.Sprintln("debug", v))
		}
		console("debug", v)
	}
}
...
func _info(v ...interface{}) {
	...
	if logLevel <= INFO {
		if logObj != nil {
			logObj.lg.Output(3, fmt.Sprintln("info", v))
		}
		console("info", v)
	}
}
...
func _error(v ...interface{}) {
	...
	if logLevel <= ERROR {
		if logObj != nil {
			logObj.lg.Output(3, fmt.Sprintln("error", v))
		}
		console("error", v)
	}
}
...
func _fatal(v ...interface{}) {
	...
	if logLevel <= FATAL {
		if logObj != nil {
			logObj.lg.Output(3, fmt.Sprintln("fatal", v))
		}
		console("fatal", v)
	}
}
